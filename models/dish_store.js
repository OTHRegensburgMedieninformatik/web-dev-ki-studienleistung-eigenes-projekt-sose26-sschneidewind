...
const dish_store = {
    ...
    async get_top_dishes(user_id) {
        ...
        :
        "with user_row as (select * from users where id=$1) select row_number() over (order by avg(stars) desc nulls last) as rank, dishes.r_id, dishes.d_id, avg(stars) as stars, dishes.name as d_name, restaurants.name as r_name, restaurants.city from dish_ratings right join dishes on (dish_ratings.r_id = dishes.r_id and dish_ratings.d_id = dishes.d_id) join restaurants on dishes.r_id = restaurants.id join user_row on (restaurants.postal_code = user_row.postal_code or restaurants.city = user_row.city) group by dishes.d_id, dishes.r_id, restaurants.name, restaurants.city order by stars desc nulls last limit 5;"
